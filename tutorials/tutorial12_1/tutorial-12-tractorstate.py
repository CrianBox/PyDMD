...
def get_dataset(samples, noise=1):
    ...
    if noise:
        ...
        noise_Y = np.random.normal(0,0.1, size=DataU.shape)
        
        DataX_w_noise = DataX + np.concatenate(([noise_X_0], [noise_X_1]), axis=0)
        DataU_w_noise = DataU + noise_Y
        
        dataset = {
            'X_real' :  DataX,
            'Y_real' :  np.array([DataU]),
            'X' : DataX_w_noise,
            'Y' :  np.array([DataU]) + np.random.normal(0,0.1, size=DataU.shape),
            'X_noise' :  np.concatenate(([noise_X_0], [noise_X_1])),
            'Y_noise' : noise_Y,
            }
    else:
        dataset = {
...
#%%
dataset = get_dataset(100, noise=1)
...
eig_real = np.linalg.eig(A)
print('Real eigenvalue {}: distance from unit circle {}'.format(eig_real[0], np.abs(np.linalg.eig(A)[0].imag**2+np.linalg.eig(A)[0].real**2 - 1))
print('Real eigenvalue {}: distance from unit circle {}'.format(eig_real[1], np.abs(np.linalg.eig(A)[0].imag**2+np.linalg.eig(A)[0].real**2 - 1))
dmdc.plot_eigs()
